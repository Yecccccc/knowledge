...
1. ClientHello
    ...
    客户端发起加密通信请求，客户端发送tls版本、随机数（client random）、客户端支持的密钥套件列表（如rsa加密算法）

2. SeverHello

    服务器收到客户端请求后，向客户端发出响应，包含：确认tls版本、随机数（server random）、确认的密码套件列表、服务器的数字证书
    
3. 客户端回应

    客户端收到服务器会回应后，首先通过浏览器或者操作系统的CA公钥、确认服务器数字证书的真实性。

    确认真实性之后，从证书取出公钥然后用它加密报文，向服务器发送：随机数（pre-master key）、加密通信算法改变的通知、客户端握手结束通知。

    **服务器和客户端有了三个随机数client random, server random, pre-master key，接着就用双方协商的加密算法，各自生成本次通信的会话密钥**

4. 服务器最后回应

    服务器收到客户端的第三个随机数后，通过加密算法算出会话密钥，最后发送：加密通信算法改变通知、服务器握手结束通知。
...
6. http/1.0 -> http/1/1 -> http/2 -> http/3
   **http1.1的性能改进**：
   - 长连接代替短连接
   - 管道网络传输，不必等待每次请求回来，就可以发送后面的请求,**解决了请求的队头阻塞**
   **http2的性能改进**：
   - 头部压缩：如果同时发出的多个请求，他们头部如果是一样或者相似，协议会消除重复的部分
   - 二进制格式：二进制格式，头信息帧和数据帧，增加传输效率
   - 并发传输：针对不同的http请求用独一无二的stream id来区分，接收方根据stream id有序组装http。可以实现stream级别的并发，**解决了http的队头阻塞**
   - 服务器推送：服务器可以主动向客户端发送消息
   **http3的性能改进**
   - 无队头阻塞：当某个流发生丢包时，只会阻塞这个流，其他流不会收到影响，因此不存在队头阻塞问题（为每个stream分配了滑动窗口）
   - 更快的连接建立：1RTT握手，第二次连接，0RTT
   - 连接迁移：以前的tcp用四元组标识连接，quic用64位的连接id标记连接

  socket与tcp握手/挥手

## tcp keep alive与http keep alive

1. tcp keepalive

   **双方长时间未通讯时，如何证明TCP连接正常并且有通讯能力？**

   如果在一段时间（tcp_keepalive_time）内连接都不活跃，开启保活功能的一端会向对端发送一个保活探测报文

   - 如果对端正常存活，且连接有效，对端必然能够收到探测报文进行响应，此时证明TCP连接正常，重置保活时间计数器
   - 如果发送端没有收到响应，经过一个探测时间间隔（tcp_keepalive_intvl）后继续发送报文，直到对端响应或者达到探测次数上限（tcp_keepalive_probes)

2. http keepalive

​		http1.0是客户端每一次请求都要和服务器端建立tcp连接，服务器响应后，断开tcp连接。

​		keep-alive机制，在http请求头设置“connection：keep-alive”进行开启。开启后，服务器响应后不再断开TCP连接，复用该连接。